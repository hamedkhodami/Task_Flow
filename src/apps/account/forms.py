...
class VerifyPhoneNumberForm(forms.Form):
    ...
    def clean(self):
        try:
            user = UserModel.objects.get(token=self.cleaned_data['token'])
        except (UserModel.DoesNotExist, TypeError, KeyError):
...
class GetPhoneNumberForm(forms.Form):
    ...
    def clean(self):
        try:
            ...
            user = UserModel.objects.get(phone_number=self.cleaned_data['phone_number'])
        except (UserModel.DoesNotExist, TypeError, KeyError):
